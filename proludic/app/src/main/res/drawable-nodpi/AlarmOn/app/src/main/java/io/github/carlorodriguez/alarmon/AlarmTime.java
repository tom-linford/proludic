...
public final class AlarmTime implements Parcelable, Comparable<AlarmTime> {
  ...
  String localizedString(Context context, boolean displayAMOrPM) {
  ...
  String timeUntilString(Context context) {
    Calendar now = Calendar.getInstance();
    if (calendar.before(now)) {
    ...
    String value = "";
    if (days == 1) {
      value += context.getString(R.string.day, days) + " ";
    } else if (days > 1) {
      value += context.getString(R.string.days, days) + " ";
    }
    if (hours == 1) {
      value += context.getString(R.string.hour, hours) + " ";
    } else if (hours > 1) {
      value += context.getString(R.string.hours, hours) + " ";
    }
    if (minutes == 1) {
      value += context.getString(R.string.minute, minutes) + " ";
    } else if (minutes > 1) {
      value += context.getString(R.string.minutes, minutes) + " ";
    }
    return value;
  }
